...
public class RUMDbSearchEngine {
	...
	public void insertMoviesIntoHashTable(String inputFile) {
		...
		while (j < movieList.size()) {
			ArrayList<String> movieWords = movieList.get(j);
			for (int i = 1; i < movieWords.size(); i++) {
				movieWords.set(i, isWord(movieWords.get(i)));
			}
			for (int i = 1; i < movieWords.size(); i++) {
				// System.out.println(movieWords.get(i));
				if (movieWords.get(i) != null) {
					Location l = new Location(movieWords.get(0), i);
					insertWordLocation(movieWords.get(i), l);
				}
			}
			j++;
		}
	...
	public void insertWordLocation(String word, Location loc) {
		...
		if (getWordOccurrence(isWord(word)) != null) {
			...
			int count = hashFunction(word);
			newOcc.next = hashTable[count];
			hashTable[count] = newOcc;
			wordCount++;
		}
	...
	private void rehash(int newHashSize) {
		...
		if (getLoadFactor() > threshold) {
			WordOccurrence[] ht = new WordOccurrence[newHashSize];
			...
			for (int i = 0; i < hashTable.length; i++) {
				...
				while (ptr != null) {
					...
					int count = hashFunction(ptr.getWord());
					ptr.next = ht[count];
					ht[count] = ptr;
					ptr = hashTable[i];
				}
	...
	public ArrayList<MovieSearchResult> createMovieSearchResult(String wordA, String wordB) {
		...
		ArrayList<Location> wordAlocation = a.getLocations();
		ArrayList<Location> wordBlocation = b.getLocations();
		// wordA
		for (int i = 0; i < wordAlocation.size(); i++) {
			Location temp = wordAlocation.get(i);
			MovieSearchResult wordsA = new MovieSearchResult(temp.getTitle());
			wordsA.addOccurrenceA(temp.getPosition());

			while (i + 1 < wordAlocation.size() && wordAlocation.get(i + 1).getTitle().equals(temp.getTitle())) {
				wordsA.addOccurrenceA(wordAlocation.get(i++).getPosition());
			}

			for (Location l : wordBlocation) {
				if (l.getTitle().equals(temp.getTitle())) {
					wordsA.addOccurrenceB(l.getPosition());
				}
			}
			list.add(wordsA);
		}
		...
		for (Location l : wordBlocation) {
			int x = 0;
			for (MovieSearchResult remain : list) {
				if (l.getTitle().equals(remain.getTitle())) {
					x++;
				}
			}
			if (x == 0) {
				MovieSearchResult temp = new MovieSearchResult(l.getTitle());
				temp.addOccurrenceA(l.getPosition());
				list.add(temp);
			}
		}
	...
	public void calculateMinDistance(MovieSearchResult msr) {
		...
		if (msr.getArrayListA() == null || msr.getArrayListB() == null || msr.getArrayListA().size() == 0
		...
		int b = 0;
		int current = min;
		int indexa = wordA.get(a);
		int indexb = wordB.get(b);
		while (a < wordA.size() && b < wordB.size()) {
			current = Math.abs(indexa - indexb);
			if (min > current) {
				min = current;
			}
			if (a < wordA.size() - 1 && b < wordB.size() - 1) {
				if (indexa < indexb) {
					indexa = wordA.get(++a);
				} else {
					indexb = wordB.get(++b);
	...
	public ArrayList<MovieSearchResult> topTenSearch(String wordA, String wordB) {
		...
		ArrayList<MovieSearchResult> resultList = new ArrayList<MovieSearchResult>();
		if (list == null) {
			return null;
		}
		for (MovieSearchResult i : list) {
			calculateMinDistance(i);
		}
		Collections.sort(list);
		int count = 0;
		while (count < 10 && count != list.size()) {
			if (list.get(count).getMinDistance() != -1)
				resultList.add(list.get(count));
			count++;
		}
		return resultList;
	}
